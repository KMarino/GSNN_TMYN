...
opt = {
    ...
    name = 'graph_vgonly_VG',               -- Name of experiment
    ...
    gsnn_sgd = 0,                       -- 1 - use sgd. Otherwise ADAM
}
...
function fevalclass(x)
    ...
    for i = 1, opt.batchSize do
        ...
        for j = 1, #active_idx do
            ...
            local output_idx = graph.nodes[full_graph_idx].vocab_index
    ...
    -- Backward pass   
    local dout = net:backward({vgg_out, detect_annotation, graph_data}, df_do_c)
...
function fevalgsnn(x)
    ...
    for i = 1, opt.batchSize do
        ...
        local importance_gt_orig_idx = graph:getDiscountedValues(target_nodes, opt.gamma, opt.num_steps)
        local importance_gt = {}
