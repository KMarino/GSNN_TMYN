...
opt = {
    ...
    name = 'baseline_COCO',               -- Name of experiment
    ...
    runmode = 'baseline',               -- baseline - just image input, detect - image + detections, graph - image + detections + gsnn
    detectMode = 'coco',                -- Which detectors to use
    lossweight = 20,
...
if opt.load_net_type == 'VGG' then
    ...
    local fc7_drop = nn.Dropout(0.5)(vgg_input)
    local fc8 = nn.Linear(fc7_size, output_size)(fc7_drop)
    fc8.data.module.weight:normal(0, 0.001);
    fc8.data.module.bias:fill(-6.58);
    local out = cudnn.Sigmoid()(fc8)
    net = nn.gModule({vgg_input}, {out}) 
end
net:cuda()
net:training()
vgg_net:cuda()
...
function fevalclass(x)
    ...
    local imdata, lab, labinds = data:getBatch()
    data_tm:stop()
    ...
    -- Backward pass   
    local dout = net:backward(vgg_out, df_do_c)
